...
Anomaly detection on journal entries runs four deterministic rules: round-number outliers (exact thousand multiples above 3x the period median), weekend posts (entries dated Saturday or Sunday), just-under-threshold amounts within 5 percent below the configured approval threshold (a standard structuring control, opt-in via the threshold), and reversal-pair candidates that group same-user, same-account, same-amount, same-day entries as possible wash or duplicate posts. Each rule has self-suppression guards so it does not fire on meaningless input.  # noqa: E501

Variance commentary produces a plain-English headline for the close pack from a budget-vs-actual snapshot: it names the direction of the total variance (overran, underran, or on budget), highlights the top-N largest movers by absolute variance, and always emits a line even on an empty or all-zero period. With a live provider configured, this hook genuinely swaps in the provider's natural-language paragraph in place of the template.  # noqa: E501

Next-best-action suggestion picks the next collections step from an eight-branch deterministic dunning ladder, where an active promise-to-pay takes precedence over the days-overdue escalation so collections never get over-aggressive against a partner who has committed to pay.  # noqa: E501

A pluggable provider registry selects the provider per company through a configuration field (default manual). Provider stubs for hosted cloud LLM APIs and for self-hosted, OpenAI-compatible local endpoints self-register at import, validate their credentials, and refuse live calls until a separate paid provider extension re-registers under the same key. Every capability catches a provider failure and falls back to its deterministic result, so a missing or bad key never breaks the call site.  # noqa: E501

Anomaly detection is wired live into Odoo: each finding is stored against the offending journal entry as an eh.account.anomaly.finding record with an immutable evidence trail and an open / reviewed / dismissed triage state. A Scan anomalies button on the entry, a list-view bulk action, and a daily per-company scheduled scan (savepoint isolated) keep the findings current. Variance commentary and the collections next-action ladder remain reusable helper functions for now, surfaced by their consuming suite modules rather than wired here.""",  # noqa: E501
    'author': 'ERP Heritage',
