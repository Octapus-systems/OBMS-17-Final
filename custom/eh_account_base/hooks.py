...
def post_init_hook(env):
    ...
    if upstream_user:
        candidates = Users.search([
            ('groups_id', 'in', upstream_user.id),
        ])
        if candidates:
            user_group.sudo().write({'users': [(4, u.id) for u in candidates]})
            _logger.info(
    ...
    if upstream_manager:
        candidates = Users.search([
            ('groups_id', 'in', upstream_manager.id),
        ])
        if candidates:
            manager_group.sudo().write({'users': [(4, u.id) for u in candidates]})
            _logger.info(
                "eh_account_base post_init: promoted %d users to "
